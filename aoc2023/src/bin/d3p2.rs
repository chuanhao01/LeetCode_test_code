use std::{
    fmt::Debug,
    fs::File,
    io::{Read, Result}, collections::HashSet,
};
...
impl No {
    ...
    fn is_in(&self, y: i64, x: i64)-> bool{
        self.y == y && self.start <= x && x < self.end
    }
}
...
fn main() -> Result<()> {
    ...
    for (y, l) in map.iter().enumerate(){
        for (x, c) in l.iter().enumerate(){
            if c == "*"{
                let indexes = get_around_index(map.clone(), y as i64, x as i64);
                let mut valid_nos = HashSet::new();
                for index in indexes{
                    for no in &nos{
                        if no.is_in(index.0, index.1){
                            valid_nos.insert(no.body);
                        }
                    }
                }
                if valid_nos.len() == 2{
                    let mut valid_nos = valid_nos.iter();
                    let n1 = valid_nos.next().unwrap();
                    let n2 = valid_nos.next().unwrap();
                    sum += n1 * n2;
                }
            }
        }
